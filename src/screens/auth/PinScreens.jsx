...
const Numpad = ({ onPress, theme, bioAvailable, onBiometric }) => (
  <View style={{ flexDirection: 'row', flexWrap: 'wrap', width: 240, alignSelf: 'center' }}>
    {KEYS.map((k, i) => {
      const isBio = k === '' && bioAvailable && !!onBiometric;
      const isDel = k === 'del';
      const isEmpty = k === '' && !isBio;

      return (
        <TouchableOpacity
          key={i}
          onPress={() => {
            if (isBio) { onBiometric?.(); return; }
            if (k) onPress(k);
          }}
          disabled={isEmpty}
...
export function PinLoginScreen({ route, navigation }) {
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: theme.bg, alignItems: 'center', justifyContent: 'center', padding: 24 }}>
      {/* Avatar */}
      <Avatar name={profile?.name ?? '?'} size={64} style={{ marginBottom: 14 }} />

      <Text style={{ fontSize: 18, fontWeight: '700', color: theme.text, marginBottom: 4 }}>
        {tr('welcomeBack')}{firstName ? `, ${firstName}` : ''}
      </Text>
      ...
      <Numpad onPress={handleKey} theme={theme} bioAvailable={bioAvail} onBiometric={handleBio} />
